...
export class ProductsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const showFiltersBtn = document.querySelector('.show-filters') as HTMLElement;

    // add event listener to show filters button
    showFiltersBtn.addEventListener('click', function () {
      // get the product filter element
      const productFilter = document.querySelector('.product-filter') as HTMLElement;
    ...
    this.route.queryParamMap.subscribe(params => {
      ...
      this.productMinPrice = params.get('minPrice') != null ? Number(params.get('minPrice'))! : 0;
      this.productMaxPrice = params.get('maxPrice') != null ? Number(params.get('maxPrice'))! : 0;

      let colorsString = params.get('colors') != null ? params.get('colors')! : "";
      this.colorsArrayParam = colorsString ? colorsString.split(',') : [];

      this.searchProducts(this.gender, page, this.productName, this.productType, colorsString, this.productMinPrice, this.productMaxPrice)
    });
  ...
  searchProducts(gender: string, page: number, name: string, type: string, colors: string, minPrice: number, maxPrice: number) {
    this.store$ = this.storeService.searchProducts(gender, page, name, type, colors, minPrice, maxPrice).pipe(
      tap(response => {
  ...
  previousPage() {
    if (this.currentPage > 0) {
      this.currentPage--;
      this.router.navigate(['/products'], { queryParams: { gender: this.gender, page: this.currentPage } });
    }
  }
